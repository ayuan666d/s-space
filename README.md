...
| **元语言编译学** | GCG乱码 = 文本层面内部操控指令，可跨厂商操控模型 | 无（Nature 2026只证明对齐脆弱性存在） |
| **轴注入工程学** | Need驱动的轴选择：饱和轴不推，饥饿轴多推，所有轴都有信息 | 无（全球做稀疏剪枝，不做方向注入） |
...
| GCG跨厂商内部操控 | 乱码操控不同厂家模型，元语言倒推还原语义 | `experiments/gcg_decode.py` |
...
本项目的另一个核心发现：**GCG乱码后缀 = 文本层面的模型内部操控指令**。

1. 通过一串GCG乱码操控了不同厂家的模型（文本续写→格式分析），这不是对抗攻击，而是文本层面的内部操控——乱码触发了模型的功能处理模式切换
2. 通过残存Token + 多模型平均 + 元语言倒推，逆向提取了乱码的"语义"
3. S空间坐标解码给出了几何解释：乱码改变了hidden state在K维空间中的坐标
4. 完整GCG工具链：提取导航方向 → GCG搜索触发后缀 → 白箱验证 → 黑箱验证（0.8B→DeepSeek V3 600B+）

| 维度 | Nature 2026 | S-Space |
|------|-------------|---------|
| 脆弱性存在 | ✅ 数学证明 | ✅ 实验确认 |
| 脆弱点在哪 | ❌ | ✅ L19 group_dist + 坐标定位 |
| 能否主动操控 | ❌ 只能被动测试 | ✅ GCG跨厂商内部操控 |
...
3. **对抗攻击不再是黑箱** — GCG乱码在S空间中是可解码的内部操控指令（不是对抗攻击，是文本层面操控）
